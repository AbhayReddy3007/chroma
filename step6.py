...
def run_step6(
    ...
    # Pre-filter diagnostic — always printed so the user can verify column values
    for key, label in [("tag", "Tag"), ("type_", "Type"), ("jurisdiction", "Jurisdiction")]:
        actual_col = col.get(key, "")
        if actual_col in df_raw.columns:
            vals = df_raw[actual_col].unique().tolist()
            print(f"[Step 6]   {label} values in CSV: {vals}")
        else:
            print(f"[Step 6]   WARNING: column for '{label}' not found. "
                  f"Tried: {_COL_ALIASES[key]}. "
                  f"Available: {list(df_raw.columns)}")

    chartable_rows = []
    for _, row in df_raw.iterrows():
        tag  = str(g(row, "tag")).strip().upper()
        typ  = str(g(row, "type_")).strip().upper()   # normalise to upper
        ...
        if not INCLUDE_FORECASTED and typ != "EXISTING":
    ...
    if df.empty:
        # Diagnostic: show unique values for key filter columns so the user
        # can see exactly why every row was dropped.
        for key, label in [("tag", "Tag"), ("type_", "Type"), ("jurisdiction", "Jurisdiction")]:
            actual_col = col.get(key, "")
            if actual_col in df_raw.columns:
                vals = df_raw[actual_col].unique().tolist()
                print(f"  [DIAG] \'{actual_col}\' unique values: {vals}")
            else:
                print(f"  [DIAG] Column for \'{label}\' not found. "
                      f"Tried: {_COL_ALIASES[key]}. "
                      f"Available columns: {list(df_raw.columns)}")
        print(f"  [DIAG] Dropped counts: {dropped}")
        return {
            "__global__": {
